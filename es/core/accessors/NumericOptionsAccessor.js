...
export class NumericOptionsAccessor extends FilterBasedAccessor {
    ...
    setOptions(titles) {
        if (titles.length === 1) {
            ...
            const keys = map(filter(this.options.options, (opt) => includes(titles, opt.title)), 'key');
    ...
    buildSharedQuery(query) {
        ...
        const filterRanges = map(filters, (filter) => this.fieldContext.wrapFilter(RangeQuery(this.options.field, {
            gte: filter.from,
            lt: filter.to
        })));
    ...
    getRanges() {
        return compact(map(this.options.options, (range) => omitBy({
            key: range.title,
            from: range.from,
            to: range.to
        }, isUndefined)));
